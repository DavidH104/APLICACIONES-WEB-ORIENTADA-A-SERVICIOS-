// Script para crear la base de datos mundial2026 en MongoDB usando mongosh
...
for (const item of collections) {
  if (db.getCollectionNames().includes(item.name)) {
    db.getCollection(item.name).drop();
  }
  db.createCollection(item.name, { validator: item.validator });
}
...
const continentResult = db.getCollection('continentes').insertMany(continents);
const groupResult = db.getCollection('grupos').insertMany(groups);
const stadiumResult = db.getCollection('estadios').insertMany(stadiums);
const phaseResult = db.getCollection('fase_final').insertMany(finalPhases);
const userResult = db.getCollection('usuarios').insertMany(users);
...
const selectionResult = db.getCollection('selecciones').insertMany(selectionsToInsert);
...
const groupDefinitions = [
  { nombre: 'A', teams: ['México', 'Inglaterra', 'Senegal', 'Australia'], stadium: 'Estadio Azteca', dates: ['2026-06-11T20:00:00Z', '2026-06-12T18:00:00Z', '2026-06-16T20:00:00Z', '2026-06-17T18:00:00Z', '2026-06-21T20:00:00Z', '2026-06-22T18:00:00Z'] },
  { nombre: 'B', teams: ['Estados Unidos', 'Alemania', 'Marruecos', 'Japón'], stadium: 'MetLife Stadium', dates: ['2026-06-11T18:00:00Z', '2026-06-12T20:00:00Z', '2026-06-16T18:00:00Z', '2026-06-17T20:00:00Z', '2026-06-21T18:00:00Z', '2026-06-22T20:00:00Z'] },
  { nombre: 'C', teams: ['Canadá', 'Francia', 'Egipto', 'Irán'], stadium: 'BC Place', dates: ['2026-06-13T18:00:00Z', '2026-06-14T20:00:00Z', '2026-06-18T18:00:00Z', '2026-06-19T20:00:00Z', '2026-06-23T18:00:00Z', '2026-06-24T20:00:00Z'] },
  { nombre: 'D', teams: ['Costa Rica', 'España', 'Ghana', 'Catar'], stadium: 'AT&T Stadium', dates: ['2026-06-13T20:00:00Z', '2026-06-14T18:00:00Z', '2026-06-18T20:00:00Z', '2026-06-19T18:00:00Z', '2026-06-23T20:00:00Z', '2026-06-24T18:00:00Z'] },
  { nombre: 'E', teams: ['Panamá', 'Portugal', 'Nigeria', 'Uzbekistán'], stadium: 'Mercedes-Benz Stadium', dates: ['2026-06-15T18:00:00Z', '2026-06-16T18:00:00Z', '2026-06-20T18:00:00Z', '2026-06-21T18:00:00Z', '2026-06-25T18:00:00Z', '2026-06-26T18:00:00Z'] },
  { nombre: 'F', teams: ['Honduras', 'Países Bajos', 'Camerún', 'Corea del Sur'], stadium: 'Hard Rock Stadium', dates: ['2026-06-15T20:00:00Z', '2026-06-16T20:00:00Z', '2026-06-20T20:00:00Z', '2026-06-21T20:00:00Z', '2026-06-25T20:00:00Z', '2026-06-26T20:00:00Z'] },
  { nombre: 'G', teams: ['Jamaica', 'Bélgica', 'Argelia', 'Emiratos Árabes Unidos'], stadium: 'SoFi Stadium', dates: ['2026-06-17T18:00:00Z', '2026-06-18T18:00:00Z', '2026-06-22T18:00:00Z', '2026-06-23T18:00:00Z', '2026-06-27T18:00:00Z', '2026-06-28T18:00:00Z'] },
  { nombre: 'H', teams: ['El Salvador', 'Croacia', 'Túnez', 'Colombia'], stadium: 'NRG Stadium', dates: ['2026-06-17T20:00:00Z', '2026-06-18T20:00:00Z', '2026-06-22T20:00:00Z', '2026-06-23T20:00:00Z', '2026-06-27T20:00:00Z', '2026-06-28T20:00:00Z'] },
  { nombre: 'I', teams: ['Uruguay', 'Italia', 'Costa de Marfil', 'Arabia Saudita'], stadium: 'Gillette Stadium', dates: ['2026-06-19T18:00:00Z', '2026-06-20T18:00:00Z', '2026-06-24T18:00:00Z', '2026-06-25T18:00:00Z', '2026-06-29T18:00:00Z', '2026-06-30T18:00:00Z'] },
  { nombre: 'J', teams: ['Ecuador', 'Suiza', 'Escocia', 'Nueva Zelanda'], stadium: 'Levi`s Stadium', dates: ['2026-06-19T20:00:00Z', '2026-06-20T20:00:00Z', '2026-06-24T20:00:00Z', '2026-06-25T20:00:00Z', '2026-06-29T20:00:00Z', '2026-06-30T20:00:00Z'] },
  { nombre: 'K', teams: ['Perú', 'Dinamarca', 'Polonia', 'Argentina'], stadium: 'BMO Field', dates: ['2026-06-21T18:00:00Z', '2026-06-22T18:00:00Z', '2026-06-26T18:00:00Z', '2026-06-27T18:00:00Z', '2026-07-01T18:00:00Z', '2026-07-02T18:00:00Z'] },
  { nombre: 'L', teams: ['Brasil', 'Gales', 'Serbia', 'Austria'], stadium: 'Estadio BBVA', dates: ['2026-06-21T20:00:00Z', '2026-06-22T20:00:00Z', '2026-06-26T20:00:00Z', '2026-06-27T20:00:00Z', '2026-07-01T20:00:00Z', '2026-07-02T20:00:00Z'] }
];
...
const cutoffDate = new Date('2026-06-25T23:59:59Z');
const matches = [];
for (const groupDef of groupDefinitions) {
  for (let index = 0; index < groupMatchPairs.length; index++) {
    ...
    const result = realMatchResults[resultKey] || { goles_local: (index % 3) + 1, goles_visitante: ((index + 1) % 3) };
    match.goles_local = result.goles_local;
    match.goles_visitante = result.goles_visitante;
    matches.push(match);
  }
}
matches.sort((a, b) => a.fecha - b.fecha);
...
const partidoResult = db.getCollection('partidos').insertMany(matchesToInsert);
...
if (ticketsToInsert.length) {
  db.getCollection('boletos').insertMany(ticketsToInsert);
}
...
for (let i = 0; i < matches.length; i++) {
  ...
  const localGroupId = groupByName[Object.keys(groupByName).find(k => k === originalMatch.equipo_local ? true : false)];
  const visitanteGroupId = groupByName[Object.keys(groupByName).find(k => k === originalMatch.equipo_visitante ? true : false)];
}

// Recalcular clasificaciones usando el grupo de cada selección de forma explícita
const groupIdBySelectionName = {};
for (const selection of selections) {
  groupIdBySelectionName[selection.nombre] = groupByName[selection.grupo];
}
...
print('Base de datos mundial2026 creada correctamente.');
printjson({
  collections: db.getCollectionNames(),
  counts: {
    continentes: db.getCollection('continentes').countDocuments(),
    grupos: db.getCollection('grupos').countDocuments(),
    selecciones: db.getCollection('selecciones').countDocuments(),
    estadios: db.getCollection('estadios').countDocuments(),
    fase_final: db.getCollection('fase_final').countDocuments(),
    partidos: db.getCollection('partidos').countDocuments(),
    clasificaciones: db.getCollection('clasificaciones').countDocuments(),
    usuarios: db.getCollection('usuarios').countDocuments(),
    boletos: db.getCollection('boletos').countDocuments()
  }
});
